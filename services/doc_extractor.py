...
_HEADER_TOKENS = {
    "Drug Name", "NDC", "Lot No/Exp Date", "Qty Disp", "Days",
    "Date Filled", "Pat Name", "Price", "RxNo", "Ref#", "Pres Name",
    "Pres Addr", "Pres Phone", "Pat Addr", "Pat Ph#", "Ins Paid",
    "Ins Code", "Qty Ord", "Inventory Bucket",
    "Page#:", "Date:", "Drug Dispensed Report",
}
...
_DATE_RANGE_RE = re.compile(
    r"Drug Dispensed Report\s+([\d/]+)\s+To\s+([\d/]+)", re.IGNORECASE
)
...
class _DispenseRecord:
    ...
    __slots__ = (
        "pat_name", "drug_name", "ndc", "qty_disp", "days_supply",
        "date_filled", "rx_no", "ref", "pres_name", "price", "ins_paid",
        "pat_addr", "pres_addr", "pat_phone", "pres_phone",
        "ins_code", "qty_ord",
        "_addr_count", "_phone_count",
    )
...
def _parse_cells(cells: list[str]) -> dict[str, Any]:
    ...
    while i < n:
                ...
                for (d_name, n_key), med in medicines_map.items():
                    if n_key == ndc:
                        ...
                        if t["total_rx_count"] is None:
                            if packs: t["packs"] = packs
                            if rx_count_t: t["total_rx_count"] = rx_count_t
                            if total_price_t: t["total_price"] = _strip_money(total_price_t)
                            if total_ins_t: t["total_ins_paid"] = _strip_money(total_ins_t)
                            if total_cost_t or total_cost: t["total_cost"] = _strip_money(total_cost_t or total_cost)
        ...
        if (
            ...
            and cell not in (_TOTALS_START, _GRAND_TOTAL, _TOTAL_FOR,
                             _PACKS_LABEL, _TOTAL_INS, _TOTAL_PRICE, _TOTAL_COST)
            ...
            if j < n and not _MONEY_RE.match(cells[j]):
                rec.pres_name = cells[j]; j += 1

            # price
            if j < n and _MONEY_RE.match(cells[j]):
                rec.price = cells[j]; j += 1

            # ins_paid
            if j < n and _MONEY_RE.match(cells[j]):
                rec.ins_paid = cells[j]; j += 1

            # pat_addr (first address)
            if j < n and _is_address(cells[j]):
                rec.pat_addr = cells[j]; j += 1

            # pres_addr (second address)
            if j < n and _is_address(cells[j]):
                rec.pres_addr = cells[j]; j += 1

            # phone (10 digits = pat_phone)
            if j < n and _PHONE_RE.match(cells[j]):
                rec.pat_phone = cells[j]; j += 1
            ...
            if j < n and not _QTY_RE.match(cells[j]) and not _PHONE_RE.match(cells[j]) \
                    and not _INT_RE.match(cells[j]) and cells[j] not in (
                        _TOTALS_START, _GRAND_TOTAL) \
                    and len(cells[j]) <= 15 and " " not in cells[j]:
                rec.ins_code = cells[j]; j += 1

            # pres_phone (10 digits)
            if j < n and _PHONE_RE.match(cells[j]):
                rec.pres_phone = cells[j]; j += 1

            # qty_ord
            if j < n and _QTY_RE.match(cells[j]):
                rec.qty_ord = cells[j]; j += 1
...
def _parse_docx_cells(cells: list[str]) -> dict[str, Any]:
    ...
    grand_total: dict[str, str | None] = {
        "total_price": None, "total_rx_count": None, "total_cost": None
    }
    ...
    while i < n:
            ...
            while j < n:
                ...
                if c2 == _TOTAL_PRICE:
                    ...
                    if len(parts) > 1:
                        ...
                        j += 1
                j += 1
                if grand_total["total_price"] and grand_total["total_cost"] and grand_total["total_rx_count"]:
                    break
            i = j
            continue
        ...
        if _NDC_RE.match(cell):
            ...
            for k in range(i, min(n, i + 15)):
                c2 = cells[k]
                if c2 == _TOTALS_START or c2.startswith(_TOTALS_START) or c2 in (_PACKS_LABEL, "Total Price: Total Cost:", _TOTAL_COST):
                    break
            ...
            if is_dispense:
                ...
                drug_name = cells[i-2] if i > 1 and cells[i-1] in ("General", "Bucket", "C", "S") else cells[i-1]
                ndc = cell
                ...
                pat_name_parts = []
                while j < n and not _QTY_RE.match(cells[j]) and not _INT_RE.match(cells[j]) and not _DATE_RE.match(cells[j]):
                ...
                ins_code = cells[j] if j < n and not _MONEY_RE.match(cells[j]) and not _RX_RE.match(cells[j]) else None
                if ins_code: j += 1
                ...
                if med_key not in medicines_map:
                    medicines_map[med_key] = {
                        ...
                        "lot_no_exp_date": None,
                        "totals": {
                            "packs": None, "total_ins_paid": None, "total_price": None,
                            "total_cost": None, "total_rx_count": None
                        },
                        "dispenses": []
                    }
                    medicines_order.append(med_key)
                ...
                ndc_t = cell
                j = i + 1
                packs = None; total_cost = None; total_price_t = None; total_ins_t = None; rx_count_t = None
                ...
                for (d_name, n_key), med in medicines_map.items():
                    if n_key == ndc_t:
                        ...
                        if t["total_rx_count"] is None:
                            ...
                            if total_price_t: t["total_price"] = _strip_money(total_price_t)
                            if total_ins_t: t["total_ins_paid"] = _strip_money(total_ins_t)
                            if total_cost: t["total_cost"] = _strip_money(total_cost)
